...
impl DrawableCanvas{
    fn draw_on_canvas(&self, offset_x: i32, offset_y: i32){
        ...
        context.fill_rect(canvas_x - PAINT_SIZE / 2.0, canvas_y - PAINT_SIZE / 2.0, PAINT_SIZE, PAINT_SIZE);
    }
...
impl Component for DrawableCanvas{
    ...
    fn view(&self, ctx: &Context<Self>) -> Html {
        let onmousedown = ctx.link().callback(|event|DrawableCanvasMessage::MouseDown(event)); 
        let onmouseup = ctx.link().callback(|event|DrawableCanvasMessage::MouseUp(event)); 
        let onmousemove = ctx.link().callback(|event|DrawableCanvasMessage::MouseTick(event)); 
    ...
    fn rendered(&mut self, _ctx: &Context<Self>, _first_render: bool) {
        ...
        let draw_context = canvas.as_ref()
