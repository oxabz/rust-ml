...
use warp::{hyper::StatusCode, path, reply::with_status, Filter, Rejection, Reply};
...
async fn inference_worker(tasks: Receiver<(Tensor, Sender<Vec<f64>>)>) {
    ...
    while let Ok((x, s)) = tasks.recv().await {
        let y_hat = mlp.forward(&x.reshape(&[784])).sigmoid();
        let res = (0..=9)
            .into_iter()
            .map(|i| f64::from(y_hat.i(i)))
            .collect::<Vec<_>>();
...
async fn inference_controller(
    queue: async_channel::Sender<(Tensor, Sender<Vec<f64>>)>,
    data: Bytes,
    ...
    if img.size1().unwrap() != 3136 {
        return Ok(with_status(
            String::from("Expected 28x28x4 image"),
    ...
    if img.size()[1] != 28 || img.size()[2] != 28 {
        return Ok(with_status(
            "Expected 28x28 images".to_string(),
            StatusCode::BAD_REQUEST,
        ));
    }
