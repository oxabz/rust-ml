...
use quote::quote;
use syn::{Ident, parse::Parse, punctuated::Punctuated, Token, LitInt, parse_macro_input, ItemFn, parenthesized, Stmt, parse_quote};
...
impl Parse for TensorArg{
    fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
        ...
        Ok(TensorArg{ ident, dims: dims.into_iter().collect()})
    }
}
...
pub fn dims(args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    for arg in args.tensors.iter() {
        let id = &arg.ident;
        let farg = input.sig.inputs.iter().find(|arg|{
            match arg {
    ...
    for arg in args.tensors.iter() {
        for (index, dim) in arg.dims.iter().enumerate() {
            symbols_table.entry(dim.clone()).or_insert(vec![]).push((index, arg.ident.clone()))
        }
    }
